r"""CLNet：面向 Massive MIMO CSI 反馈的复数输入轻量化神经网络。

本模块从 ``CLNet/models/clnet.py`` 迁移而来，保留了原始实现的层定义、
初始化方式与前向行为。
"""
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = torch.sigmoid(x_out)  # 广播相乘
        return x * scale
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.view(n, -1)
        out = out.unsqueeze(2)  # [N, 2048, 1]
        out = self.replace_efc(out)  # [N, 2048/reduction, 1]
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.replace_dfc(x)  # [N, 2048, 1]
...
def create_clnet(reduction: int = 4) -> CLNet:
    """CLNet 工厂函数。

    :param reduction: 压缩比的倒数（CR = 1/reduction）
    """
    return CLNet(reduction=reduction)
