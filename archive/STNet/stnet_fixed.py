...
encoded_dim = 512  # 1/4->512, 1/8->256, 1/16->128, 1/32->64, 1/64->32
img_size = 32
num_heads = 4
...
# Hyperparameters (must be defined before fit() is called)
epochs = 1000
lr = 0.001
batch_size = 200
...
class Encoder(nn.Module):
    def __init__(self, img_size=img_size, depth=depth, num_heads=num_heads, qkv_bias=qkv_bias):
        super().__init__()
        self.blocks = nn.ModuleList([
            WTL(num_heads=num_heads, qkv_bias=qkv_bias) for _ in range(depth)
        ])
        self.norm2 = nn.LayerNorm(img_size, eps=1e-6)
        self.norm3 = nn.LayerNorm(img_size, eps=1e-6)
...
def fit(epochs, lr, start_idx=1):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0 or epoch == epochs - 1:
            save_path = f'./stnet_checkpoint_epoch{epoch+1}.pth'
            torch.save({
                'epoch': epoch + 1,
...
plt.ylabel("Loss")
plt.legend()
plt.savefig('./stnet_training_loss.png')
plt.close()
print('Saved training loss curve to ./stnet_training_loss.png')

del losses_auto

# Evaluation on first 1000 test samples
n = 10
encoder.eval()
decoder.eval()
with torch.no_grad():
    temp = encoder(x_test[0:1000, :, :, :])
    x_hat = decoder(temp)
...
for i in range(n):
    ...
    decoded_imgsplo = abs(x_hat_in[i, 0, :, :] - 0.5 + 1j * (x_hat_in[i, 1, :, :] - 0.5))
    plt.imshow(np.max(np.max(decoded_imgsplo)) - decoded_imgsplo.T)
    plt.gray()
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    ax.invert_yaxis()
plt.savefig('./stnet_reconstruction.png')
plt.close()
print('Saved reconstruction visualization to ./stnet_reconstruction.png')

# Save final results
with open('./stnet_result.txt', 'w') as f:
    f.write(f"STNet reproduction\n")
    f.write(f"Environment: {envir}\n")
    f.write(f"Encoded dim: {encoded_dim}\n")
    f.write(f"Epochs: {epochs}\n")
    f.write(f"Batch size: {batch_size}\n")
    f.write(f"Learning rate: {lr}\n")
    f.write(f"NMSE (dB): {nmse_db}\n")
print('Saved results to ./stnet_result.txt')
